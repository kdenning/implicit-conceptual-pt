...
contrasts(anova_error_data2$consistent) <- ConCodes
contrasts(anova_error_data2$consistent)
```
...
model2 <- with(anova_error_data2,
                  aov(error_per_group ~ (visibility_effects*consistent) +
                        Error(participant/(visibility_effects*consistent))))
...
## Plot for main effect of consistency

```{r error main effect consist}
data_aggregated_errorrate <- anova_error_data2 %>% 
  select(participant, consistent, error_per_group) %>% 
  na.omit() %>% 
  group_by(consistent) %>% 
  summarize(mean = mean(error_per_group),
            sd = sd(error_per_group),
            se = sd(error_per_group)/sqrt(length(error_per_group)))
...
```{r}
# Running logistic regression predicting error
model4 <- with(anova_error_data_block3,
                  aov(error_per_group ~ (visibility_effects*consistent*expBlock.thisN) +
                        Error(participant/(visibility_effects*consistent*expBlock.thisN))))

...
```{r combining datasets and cleaning}
...
clean_sap <- function(file) {
                    ...
                    select(participant, expBlock.thisN, expTrials.thisN, 
                      ...
                      mutate(trial_num = as.numeric(row_number()),
                             consistent = as.factor(consistent),
                             visibility = as.factor(ifelse(facing_exp == showL_R, "same", 
                                       ifelse(facing_exp != showL_R, "diff", NA))))
}
...
contrasts(data$consistent)
CIUvY <- c(1, -1, -1, 3) 
PIvU <- c(-1, -1, 2, 0)  
CvI <- c(-1, 1, 0, 0) 
ConCodes <- cbind(CIUvY, PIvU, CvI)
contrasts(data$consistent) <- ConCodes
contrasts(data$consistent) 

## Effects codes for visibility variable
data %<>% mutate(visibility_effects = recode(visibility, `diff` = "-.5",
                                   `same` = ".5"))
```

```{r descriptive data}
#effects code visibility
#helmert code consistentcy

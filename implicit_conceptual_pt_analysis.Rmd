...
clean_sap <- function(file) {
                    ...
                    select(participant, expBlock.thisN, expTrials.thisN, 
                      ...
                      mutate(trial_num = as.numeric(row_number()),
                             consistent = as.factor(consistent),
                             visibility = as.factor(ifelse(facing_exp == showL_R, "Visible", 
                                       ifelse(facing_exp != showL_R, "Not Visible", NA))))
}
...
data_consistent <- data_comb %>% 
  ...
  na.omit() %>% 
  group_by(consistent) %>% 
  summarize(mean = mean(exp_resp.rt),
            sd = sd(exp_resp.rt))
data_consistent 
```
...
```{r visibility main}
ggplot(data_visible, aes(x = visibility, y = mean, fill = visibility)) +
